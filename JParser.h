 ...
 *			      HEADER
 *
 *   $Id: JParser.h 3467 2014-08-26 20:39:20Z wini $
 *
...
typedef struct JParserIntf
{
   ...
   */
   ICACHE_FLASH_ATTR JParserIntf(JParserIntf_Service s) { service = s; }
   ...
   */
   int ICACHE_FLASH_ATTR serviceCB(JParserVal* v, int recLevel);

   ICACHE_FLASH_ATTR ~JParserIntf(){}
   ICACHE_FLASH_ATTR JParserIntf() {}
#endif
...
struct JParser
{
   ...
   */
   ICACHE_FLASH_ATTR JParser(JParserIntf* intf, char* nameBuf, int namebufSize,
           AllocatorIntf* alloc, int extraStackLen=0);
   
   ICACHE_FLASH_ATTR JParser()
        {}
   
   void ICACHE_FLASH_ATTR init(JParserIntf* intf, char* nameBuf, int namebufSize,
           AllocatorIntf* alloc, int extraStackLen=0);
   ...
   /** Parse a JSON text chunk.
       ...
       \sa getStatus
   */
   int ICACHE_FLASH_ATTR parse(const U8* buf, U32 size);
   ...
   /** Returns the parser status. Typically used when method parse
       ...
       \sa parse
   */
   JParsStat ICACHE_FLASH_ATTR getStatus();
#endif
   JLexer lexer;
   JParserVal val;
...
extern "C" {
#endif
BA_API void ICACHE_FLASH_ATTR JParser_constructor(JParser* o, JParserIntf* intf, char* nameBuf,
                                int namebufSize, AllocatorIntf* alloc,
                                int extraStackLen);
BA_API int ICACHE_FLASH_ATTR JParser_parse(JParser* o, const U8* buf, U32 size);
BA_API void ICACHE_FLASH_ATTR JParser_destructor(JParser* o);
...
inline void JParser::init(JParserIntf* intf, char* nameBuf,
                        int namebufSize, AllocatorIntf* alloc,
                        int extraStackLen) {
   JParser_constructor(this, intf, nameBuf, namebufSize, alloc, extraStackLen);
}
