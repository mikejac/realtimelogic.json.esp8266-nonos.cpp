 ...
 *            PROGRAM MODULE
 *
 *   $Id: JParser.c 3474 2014-09-02 22:35:17Z wini $
 *
 *   COPYRIGHT:  Real Time Logic, 2006 - 2014
 *
...
static JLexerT ICACHE_FLASH_ATTR
   ...
   for(;;)
   {
      ...
      switch(o->state)
      {
         ...
         case JLexerSt_TrueFalseNull:
            if(tolower(*o->tokenPtr++) != *o->typeChkPtr++)
               return JLexerT_ParseErr;
            if( ! *o->typeChkPtr )
            {
               o->state = JLexerSt_GetNextToken;
         ...
         case JLexerSt_Number:
            while(os_strchr(jsonNumberChars, *o->tokenPtr))
            {
...
{
   // @mikejac
   //JParser_setStatus(o, JParsStat_Done, 0);
