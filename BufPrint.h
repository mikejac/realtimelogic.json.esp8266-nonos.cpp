...
typedef struct BufPrint
{
      ...
      */
      //ICACHE_FLASH_ATTR BufPrint(void* userData=0, BufPrint_Flush flush=0);
      ICACHE_FLASH_ATTR BufPrint()
        {}
      /** Returns the user data pointer set in the constructor.
       */
      void* ICACHE_FLASH_ATTR getUserData();
...
<a href="http://www.infosys.utas.edu.au/info/documentation/C/CStdLib.html#stdio.h">
          ...
          more information.
      */
      int ICACHE_FLASH_ATTR vprintf(const char* fmt, va_list argList);
      ...
      /** The printf function's format is identical to the standard
          ...
          C Standard Library</a> for more information.
       */
      int ICACHE_FLASH_ATTR printf(const char* fmt, ...);

      /** print character */
      int ICACHE_FLASH_ATTR putc(int c);

      /** Write data to the buffer.
       * \param data pointer to data.
       * \param len size of data.
       */
      int ICACHE_FLASH_ATTR write(const void* data, int len);
       ...
       */
      int ICACHE_FLASH_ATTR write(const char* buf);

      void ICACHE_FLASH_ATTR setFlush(BufPrint_Flush flush);
      
   /** Set the buffer used by BufPrint
    \param buf the buffer
    \param size the buffer size
   */
   void ICACHE_FLASH_ATTR setBuf(char* buf, int size);
      ...
      */
      int ICACHE_FLASH_ATTR b64Encode(const void* data, S32 slen);
      ...
      */
      int ICACHE_FLASH_ATTR jsonString(const char* str);
#endif
...
BA_API void ICACHE_FLASH_ATTR BufPrint_constructor(
   BufPrint* o,void* userData,BufPrint_Flush flush);
#define BufPrint_destructor(o)
BA_API int ICACHE_FLASH_ATTR BufPrint_vprintf(BufPrint* o, const char* fmt, va_list argList);
BA_API int ICACHE_FLASH_ATTR BufPrint_printf(BufPrint* o, const char* fmt, ...);
BA_API int ICACHE_FLASH_ATTR BufPrint_write(BufPrint* o, const void* data, int len);
BA_API int ICACHE_FLASH_ATTR BufPrint_putc(BufPrint* o, int c);
BA_API int ICACHE_FLASH_ATTR BufPrint_flush(BufPrint* o);
#define BufPrint_write2(o, data) BufPrint_write(o, data, -1)
BA_API int ICACHE_FLASH_ATTR BufPrint_b64Encode(BufPrint* o, const void* source, S32 slen);
BA_API int ICACHE_FLASH_ATTR BufPrint_jsonString(BufPrint* o, const char* str);
#ifdef __cplusplus
}
...
inline void BufPrint::init() {
   BufPrint_constructor(this, 0, 0); }
