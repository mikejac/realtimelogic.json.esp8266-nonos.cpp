 ...
 *            PROGRAM MODULE
 *
 *   $Id: BufPrint.c 3445 2014-08-11 21:32:29Z wini $
 *
 *   COPYRIGHT:  Real Time Logic, 2002 - 2012
 *
...
BufPrint_write(BufPrint* o, const void* buf, int len)
{
   ...
   if(len)
   {
      ...
      {
         os_memcpy(o->buf + o->cursor, buf, len);
         o->cursor += len;
      }
      else
      {
...
BufPrint_vprintf(BufPrint* o, const char* fmt, va_list argList)
{
   ...
   while (*fmt)
   {
      if(*fmt != '%')
      {
         ...
         while (*fmt && *fmt != '%')
            ++fmt;
         if( (retVal=BufPrint_write(o, start, fmt - start)) != 0)
            return retVal;
         continue;
      }
      else
               ...
               *bufPtr = 0;
               ...
               else
               {
                  if (flags & FLAG_POINTER)
                     val = (unsigned long)va_arg(argList, void *);
