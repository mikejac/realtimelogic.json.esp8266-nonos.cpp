 ...
 *            PROGRAM MODULE
 *
 *   $Id: JVal.c 3463 2014-08-20 21:00:52Z wini $
 *
...
JVal_extractObject(JVal* o,JErr* err,const char** fmt,va_list* argList)
{
   ...
   for( ; **fmt && **fmt != '}' && JErr_noError(err) ; (*fmt)++)
   {
      ...
      JVal* iter = o;
      while(iter && (n = JVal_getName(iter)) && os_strcmp(name,n) )
...
JParserValFact_expandStack(JParserValFact* o)
{
   ...
   {
      o->vStack = v;
      o->vStackSize=newSize/sizeof(void*);
      return 0;
   }
...
JParserValFact_service(JParserIntf* super, JParserVal* pv, int recLevel)
{
   ...
   if(recLevel >= o->vStackSize && JParserValFact_expandStack(o))
      return -1;
   if( ! (v = AllocatorIntf_malloc(o->vAlloc, &jValSize)) )
   {
...
{
   os_memset(o, 0, sizeof(JParserValFact));
   o->vAlloc=vAlloc;
   o->dAlloc=dAlloc;
