...
: public JParserIntf
{
   ...
   int ICACHE_FLASH_ATTR vget(const char* fmt, va_list* argList);
   ...
   /** Build a pointer value tree that is used by the integrated
      ...
      \sa JEncoder::set
    */
   int ICACHE_FLASH_ATTR get(const char* fmt, ...);
   ...
   */
   ICACHE_FLASH_ATTR JDecoder(U8* buf, int bufSize, int extraStackLen=0);
#if 0
}
#endif
#else
